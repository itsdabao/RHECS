    ...
    try:
        ...
        if hasattr(e, "last_attempt"):
            traceback.print_exception(
                type(e.last_attempt.exception()),
                e.last_attempt.exception(),
                e.last_attempt.exception().__traceback__,
            )
        else:
            traceback.print_exc()
        print(
            "Bạn đã chắc chắn có GEMINI_API_KEY trong file .env và đã cài đặt thư viện 'google-genai' chưa?"
        )
