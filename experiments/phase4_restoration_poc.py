...
async def restore_claim(
    draft_sentence: str, fault_span: str, error_type: str, claim_triplet: dict
):
    """
    ...
    if not evidence:
        print(
            f"[Restoration] ⚠️ No evidence found for '{fault_span}'. Cannot safely patch."
        )
        return None

    print(f"[Restoration] 🛠️ Rewriting ISO Patch for '{fault_span}' via Gemma-4-26b...")
    repair_instruction = await fix_claim(
        draft_sentence, fault_span, error_type, evidence
    )
...
async def run_phase4(mock_error_map: dict):
    ...
    for patch in repair_results:
        if patch:
            print(
                f"🔧 Applying Fix: '{patch['fault_span']}' -> '{patch['corrected_span']}'"
            )
            print(f"   [LLM Analysis]: {patch['analysis']}")
            try:
                restored_draft = surgical_replace(
                    restored_draft, patch["fault_span"], patch["corrected_span"]
                )
        ...
        "errors": [
            {
                ...
                "claim_triplet": {
                    "entity": "OpenAI ChatGPT-4 safety protocol",
            ...
            },
            {
                ...
                "claim_triplet": {
                    ...
                    "relationship": "built",
                    "target": "external frame for the Statue of Liberty",
                },
            },
        ],
    }
