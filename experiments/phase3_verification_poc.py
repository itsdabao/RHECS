...
async def verify_claim(
    original_sentence: str, claim: dict, tenant_id: Optional[str] = None
):
    print(
    ...
    for attempt in range(MAX_RETRIES):
        ...
        print(
            f"[{claim['entity']}] Root Planner generating verification Python script..."
        )
        ...
        sandbox_result = await asyncio.to_thread(
            execute_sandbox_code, script_code, tenant_id
        )
        ...
        if sandbox_result["success"]:
            ...
            print(
                f"[{claim['entity']}] Sandbox Raw Evidence JSON Extract:\n{evidence_data}"
            )
            ...
            print(
                f"[{claim['entity']}] Feeding trace back to Root Planner for Recursive Repair..."
            )
    ...
    from verification.nli_judge import ErrorCategory, NLIStatus, VerificationResult
...
async def run_phase3():
    ...
    for claim, verdict in zip(mock_payload["claims"], results):
        claim_result = {"claim": claim, "verification": verdict.model_dump()}
