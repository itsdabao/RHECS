...
def setup_mock_qdrant(tenant_id: str):
    ...
    client.upsert(
        collection_name="truth_base",
        points=[
            PointStruct(id=1, vector=[1.0, 0.0, 0.0], payload={"tenant_id": "tenant_123", "text": "Gustave Eiffel established his reputation by building the internal frame for the Statue of Liberty."}),
            PointStruct(id=2, vector=[0.0, 1.0, 0.0], payload={"tenant_id": "tenant_123", "text": "In 2023, OpenAI releases ChatGPT-4, and its safety protocol has been improved, says Sam Altman."}),
        ]
    )
